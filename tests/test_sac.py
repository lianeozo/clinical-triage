...
def _make_agent(seed=0):
    return SACAgent(obs_dim=State.NUM_STATE_VARS, n_actions=Action.NUM_ACTIONS_TOTAL,
                    config=SACAgentConfig(), seed=seed, device="cpu")
...
def test_sac_act_eval_mode_deterministic():
    agent = _make_agent()
    obs = np.zeros(State.NUM_STATE_VARS, dtype=np.float32)
...
def test_sac_update_returns_expected_keys():
    agent = _make_agent()
...
def test_sac_twin_q_min_is_lower():
    """min(Q1, Q2) <= each."""
    agent = _make_agent()
...
def test_sac_update_moves_params():
    agent = _make_agent()
    rng = np.random.default_rng(0)
